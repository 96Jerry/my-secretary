...
export class IntentParserService {
  ...
  private buildPrompt(message: string, ctx: IntentContext): string {
    return [
      ...
      `op 선택:`,
      `- set: current_state 냉장고에 없는 새 아이템이거나, 남은 수량 자체를 말할 때 ("닭가슴살 800g 샀어"인데 냉장고에 닭가슴살이 없음, "계란 10개 남았어").`,
      `- adjust: current_state 냉장고에 있는 아이템이 늘거나 줄 때. 증가는 양수, 차감은 음수 ("계란 10개 샀어"인데 냉장고에 계란이 있음 → quantity: 10, "계란 3개 먹었어" → quantity: -3).`,
      ...
      `expiresAt은 메시지에 유통기한이 명시된 경우만 ISO YYYY-MM-DD로 넣고, 없으면 null (기존 아이템의 유통기한은 유지된다).`,
